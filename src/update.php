<?php
    ...
    <br><br><br><br>
        <form class='uk-grid-small' uk-grid action='' method='POST'>
            ...
            <div class='uk-width-1-1'>
                <h2 style='text-align:center'>Você deseja realmente fechar a piramide $id?</h2>
                <br>
                <input class='uk-input' type='text' name='hse' placeholder='Digite o Comentário do Fechamento'>
                <br><br>
            </div>
    ...
    if (isset($_POST["sim"]))
    {
        $hse = $_POST["hse"];
        $delete_pir="
        update regpir set status ='Fechado', hse=:hse where id_piramide = :id
        ";
        $go_edit = header('Location: edit_pir.php');
        $comando=$db->prepare($delete_pir);
        $comando->bindValue('id', $id);
        $comando->bindValue('hse', $hse);
